...
class LandmarkLoss(torch.nn.Module):
    ...
    def forward(self, outputs, batch):
        ...
        hp_loss, hm_hp_loss, hp_offset_loss, wh_face_loss, face_kpts_loss = 0, 0, 0, 0, 0
        hand_kpts_loss = 0
        ...
        for s in range(opt.num_stacks):
            ...
            face_kpts_loss += self.crit_kp(output['face_lms'], batch['lms_face_mask'],
                                              batch['hp_ind_face_cnt'], batch['lms_face']) / opt.num_stacks

            hand_kpts_loss += self.crit_kp(output['hand_lms'][:, :42, :, :],
                                           batch['lms_hand_mask'][:, :, :42],
                                           batch['hp_ind_hand_cnt'][:, :32],
                                           batch['lms_hand'][:, :, :42]) / opt.num_stacks
            ...
            # leftfoot_kpts_loss += self.crit_kp(output['foot_lms'][:, :6, :, :],
            #                                batch['lms_foot_mask'][:, :, :6],
            #                                batch['hp_ind_foot_cnt'][:, :32],
            #                                batch['lms_foot'][:, :, :6]) / opt.num_stacks
            #
            # rightfoot_kpts_loss += self.crit_kp(output['foot_lms'][:, 6:, :, :],
            #                                batch['lms_foot_mask'][:, :, 6:],
            #                                batch['hp_ind_foot_cnt'][:, 32:],
            #                                batch['lms_foot'][:, :, 6:]) / opt.num_stacks
            ...
            if opt.wh_weight > 0:
                ...
                wh_face_loss += self.crit_reg(output['wh_face'], batch['hp_mask_face_cnt'],
                                              batch['hp_ind_face_cnt'], batch['wh_face']) / opt.num_stacks
        ...
        loss = opt.hm_weight * hm_loss + opt.wh_weight * wh_loss + \
               ...
               opt.wh_weight * wh_face_loss + opt.hp_weight * face_kpts_loss + \
               opt.hp_weight * hand_kpts_loss


        loss_stats = {'loss': loss, 'hm_loss': hm_loss, 'hp_loss': hp_loss,
                      'hm_hp_loss': hm_hp_loss, 'hp_offset_loss': hp_offset_loss,
                      'wh_loss': wh_loss, 'off_loss': off_loss, 'wh_face_loss': wh_face_loss,
                      'face_kpts_loss': face_kpts_loss, 'hand_kpts_loss': hand_kpts_loss}
...
class LandmarkTrainer(BaseTrainer):
    ...
    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'hp_loss', 'hm_hp_loss',
                       'hp_offset_loss', 'wh_loss', 'off_loss', 'wh_face_loss',
                       'face_kpts_loss', 'hand_kpts_loss']
        loss = LandmarkLoss(opt)
